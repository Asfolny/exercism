...
def meetup(year, month, week, week_day):
    if week not in START_DAY:
        raise MeetupDayException(f'What is {week}?')

    if week_day not in day_name:
        raise MeetupDayException(f'Unknown day: {week_day}')

    start_day = START_DAY[week] or monthrange(year, month)[1] - 6
