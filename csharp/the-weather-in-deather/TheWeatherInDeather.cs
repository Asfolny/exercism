...
public class WeatherStation
{
    ...
    public Outlook ShortTermOutlook => reading.Equals(new Reading()) ? throw new ArgumentException() :
        true switch {
                true when reading.Pressure < 10m && reading.Temperature < 30m => Outlook.Cool,
                true when reading.Temperature > 50 => Outlook.Good,
                _ => Outlook.Warm
        };

    public Outlook LongTermOutlook => true switch {
        true when reading.WindDirection == WindDirection.Southerly
                || reading.WindDirection == WindDirection.Easterly
                && reading.Temperature > 20 => Outlook.Good,
        true when reading.WindDirection == WindDirection.Northerly => Outlook.Cool,
        true when reading.WindDirection == WindDirection.Easterly
                && reading.Temperature <= 20 => Outlook.Warm,
        true when reading.WindDirection == WindDirection.Westerly => Outlook.Rainy,
